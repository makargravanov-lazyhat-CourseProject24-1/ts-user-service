    ...
    stages {
        ...
        stage("Deploy"){
            steps {
                ...
                sh "sudo docker run --name ${SERVICE_NAME} -p 8021:${HTTP_PORT} -e HTTP_PORT=${HTTP_PORT} -e DB_URL=${DB_URL} -e DB_USERNAME=${DB_USERNAME} -e DB_PASSWORD=${DB_PASSWORD} jetlabs/${SERVICE_NAME}:latest"
            }
        }
    }
}